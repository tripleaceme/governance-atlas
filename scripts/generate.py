...
<link rel="canonical" href="https://governance-atlas.pages.dev/countries/{slug}">
<meta property="og:type" content="website">
<meta property="og:site_name" content="Governance Atlas">
<meta property="og:title" content="{title}">
<meta property="og:description" content="{meta_desc}">
<meta property="og:url" content="https://governance-atlas.pages.dev/countries/{slug}">
<meta property="og:image" content="https://governance-atlas.pages.dev/assets/og-image.png">
<meta property="og:image:secure_url" content="https://governance-atlas.pages.dev/assets/og-image.png">
...
<meta name="twitter:image" content="https://governance-atlas.pages.dev/assets/og-image.png">
<link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 48 48'><circle cx='24' cy='24' r='22' fill='none' stroke='%23a23c2a' stroke-width='1.5' stroke-dasharray='2 3'/><circle cx='24' cy='24' r='17' fill='none' stroke='%2322304a' stroke-width='2.5'/><ellipse cx='24' cy='24' rx='8' ry='17' fill='none' stroke='%2322304a' stroke-width='1.6'/><line x1='7' y1='24' x2='41' y2='24' stroke='%2322304a' stroke-width='1.6'/><line x1='24' y1='7' x2='24' y2='41' stroke='%2322304a' stroke-width='1.6'/><circle cx='21' cy='27' r='3' fill='%23a23c2a'/></svg>">
...
<footer class="site">
  <div class="wrap">
    ...
    <div class="credits">
      ...
      <a href="mailto:hello@atlas.ayoadeabel.tech">hello@atlas.ayoadeabel.tech</a>
...
"""
    ...
    return PAGE_SHELL.format(
        ...
        code=esc(c["code"]), slug=c["slug"], lawline=stub_lawline(c), chips=chips, facts=facts, article=article,
    )
...
def main():
    ...
    # World-map dataset: ISO2 → [name, slug, status, continent_slug, law].
    # Researched status (details.json) wins over the base record where present.
    mapdata = {
        c["code"]: [
            c["name"], c["slug"],
            (details.get(c["slug"]) or {}).get("status", c["status"]),
            c["continent_slug"], c["law"],
        ]
        for c in countries
    }
    map_js = (
    ...
    (ROOT / "assets" / "map-data.js").write_text(map_js, encoding="utf-8")

    # Continent rollup + full country index, both derived from researched status.
    order = ["Africa", "Europe", "Asia", "North America", "South America", "Oceania"]
    roll = {}
    for c in countries:
        st = (details.get(c["slug"]) or {}).get("status", c["status"])
        r = roll.setdefault(c["continent"], {"slug": c["continent_slug"], "total": 0, "a": 0, "d": 0, "n": 0})
        r["total"] += 1
        r[st] += 1
    continents = [
        {"name": n, "slug": roll[n]["slug"], "total": roll[n]["total"],
         "active": roll[n]["a"], "draft": roll[n]["d"], "none": roll[n]["n"]}
        for n in order if n in roll
    ]
    index_rows = sorted(
        ({"name": c["name"], "slug": c["slug"], "code": c["code"], "continent": c["continent"],
          "status": (details.get(c["slug"]) or {}).get("status", c["status"]),
          "law": (details.get(c["slug"]) or {}).get("law") or c["law"] or ""}
         for c in countries),
        key=lambda r: r["name"],
    )
    idx_js = (
        "// Generated by scripts/generate.py — do not edit by hand.\n"
        "const CONTINENTS = " + json.dumps(continents, ensure_ascii=False, indent=2) + ";\n"
        "const COUNTRY_INDEX = " + json.dumps(index_rows, ensure_ascii=False) + ";\n"
    )
    (ROOT / "assets" / "index-data.js").write_text(idx_js, encoding="utf-8")
    print("continent rollup: " + ", ".join(f'{c["name"]} {c["active"]}a/{c["draft"]}d/{c["none"]}n' for c in continents))
    ...
    print(f"map dataset: {len(mapdata)} countries → assets/map-data.js")
